...
<mydiv>
    <div style="width: 100%;height: 100%;">
        ...
        <c:if test="${pass != 'true'}">
            ...
            <button id="noPass" class="btn btn-warning" >审核不通过</button>
        </c:if>
        <c:if test="${pass == 'true'}">
            <b>已审核</b>
        </c:if>
    ...
    <script>
        $(document).ready(function(){
            ...
            $("#confirmPass").click(function(){
                ...
                $.ajax({
                    url:"/academy/check/detail/pass/${itemId}",
                    type:'post',
                    success:function(res){
                        var json = JSON.parse(res);
                        if(json["success"] == 'true'){
                            alert("审核成功");
                            location.href="/check/academy";
                        }else{
                            alert(json["error"]);
                        }
                    }
            ...
            $("#confirmNoPass").click(function(){
                ...
                $.ajax({
                    url:"/academy/check/detail/nopass/${itemId}",
                    type:'post',
                    success:function(res){
                        var json = JSON.parse(res);
                        if(json["success"] == 'true'){
